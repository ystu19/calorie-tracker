...
const error = ref("");
let loadRequestId = 0;
const blank = () => ({ name: "", base_amount: 100, unit: "g", serving_unit: "", serving_weight_g: "", protein: "", fat: "", carbs: "", has_alcohol: false, alcohol_abv: 0, nutrition_source: "manual", estimated: false });
  ...
  try {
    await request(editingId.value ? `/api/foods/${editingId.value}` : "/api/foods", {
      ...
      body: JSON.stringify({
        ...
        serving_unit: form.serving_unit.trim() || null,
        serving_weight_g: form.serving_unit.trim() ? Number(form.serving_weight_g) : null,
